...
export default function TarotIntro({ compact }: TarotIntroProps) {
  ...
  return (
    <div
      className={`rounded-2xl border border-violet-500/20 bg-gradient-to-b from-[#1a0f2e]/90 via-[#120a22]/95 to-[#0d0818] ${
        compact ? "p-3" : "p-4"
      }`}
    >
      <div className="mb-3 flex items-center gap-2">
        <span className="flex h-8 w-8 items-center justify-center rounded-full bg-violet-500/25 text-base">🔮</span>
        <div>
          <h3 className={`font-semibold text-app-gold ${compact ? "text-xs" : "text-sm"}`}>什么是塔罗 AI？</h3>
          <p className={`text-violet-300/70 ${compact ? "text-[10px]" : "text-[11px]"}`}>韦特系塔罗 · AI 深度解读</p>
        </div>
      </div>

      <div className={`space-y-3 text-violet-200/70 ${bodyClass}`}>
        <p>
        ...
        <div>
          ...
          <p className="text-violet-200/65">
            输入问题后进入洗牌 → 凭直觉选 <strong className="font-semibold text-violet-100">3 张牌</strong> → 确认牌阵后点击 <strong className="font-semibold text-violet-100">AI解读牌阵</strong>。三牌阵分别代表 <strong className="font-semibold text-violet-100">过去 · 现在 · 未来</strong>。
          </p>
        </div>
